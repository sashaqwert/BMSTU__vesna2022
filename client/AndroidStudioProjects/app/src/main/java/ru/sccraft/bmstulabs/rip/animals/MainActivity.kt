...
class MainActivity : AppCompatActivity() {
    ...
    private val LOG_TAG = "Animals/MainActivity"
    ...
    private fun получить_данные_из_API() {
        ...
        Log.d(LOG_TAG, "Используемый URL API: $API_URL")
        NetworkService.getInstance()
            ...
            .enqueue(object : retrofit2.Callback<List<Animal?>?> {
                override fun onResponse(
                    ...
                    if (animalList != null) {
                        Log.i(LOG_TAG, "Данные из API успешно получены!")
                        ...
                        error_arrayList.add(getString(R.string.unable_get_data))
                ...
                override fun onFailure(call: Call<List<Animal?>?>, t: Throwable) {
                    //Что-то пошло не так…
                    Log.e(LOG_TAG, "Не удалось получить данные из API")
                    t.printStackTrace()
                    показать_диалог_ошибки_загрузки()
                    скрыть_индикатор_загрузки()
